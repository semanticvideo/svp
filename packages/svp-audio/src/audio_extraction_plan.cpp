...
nlohmann::json loudness_plan_to_json(const LoudnessArtifactPlan& plan) {
  ...
  return {
      ...
      {"pending_reason", "loudness measurement requires staged original audio streams"},
  };
}
...
AudioExtractionPlan build_audio_extraction_plan(const std::filesystem::path& source_path,
  ...
  plan.ffprobe_path = ffprobe_path;
  ...
  plan.loudness.depends_on = extraction_task_ids;
  ...
  for (std::size_t index = 0; index < analysis_streams.size(); ++index) {
    const svp::media::AudioStreamProbe& stream = *analysis_streams[index];
    plan.loudness.targets.push_back(LoudnessStreamTarget{
        stream.id,
        stream.index,
        stream.channels,
        stream.sample_rate,
        normalized_stream_start_us(stream, *presentation_origin),
        stream_output_ref(index),
    });
  }
