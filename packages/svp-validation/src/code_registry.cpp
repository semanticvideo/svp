...
void add_temporary_validator_codes(ValidationCodeRegistry& registry) {
  ...
  registry.add(make_temp_code(kTempCodeBlockPayloadDecodeFailed, FindingSeverity::error,
                              "A binary block payload could not be decompressed as declared."));
  registry.add(make_temp_code(kTempCodeMissingEmbeddingsEntry, FindingSeverity::error,
                              "A required embeddings package entry is absent."));
}
