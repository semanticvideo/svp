...
TimelineWriteSummary write_timeline_artifacts(
  ...
  for (std::size_t i = 0; i < sampling.shots.size(); ++i) {
  ...
  for (std::size_t i = 0; i < sampling.scenes.size(); ++i) {
    ...
    for (const auto& shot : sampling.shots) {
      ...
      if (overlaps) {
        shot_ids.push_back(shot.target_id);
      }
    }
